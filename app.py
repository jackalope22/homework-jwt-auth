...
def decode_auth_token(auth_token):
    ...
    decoded = jwt.decode(auth_token, jwt_secret_key, algorithms=['HS256'])
    return decoded
...
def user():
    ...
    client = decode_auth_token(auth_token)
    return {
        'user_id': client.get("sub"),
...
def login():
    ...
    new_token = encode_auth_token(user.get("id"), user.get("name"), user.get("email"), "no idea")
    return {
        'token': new_token
    }
